...
async def test_watch_usage_no_args(watchlist_engine) -> None:
    msg = FakeMessage()
    update = FakeUpdate(message=msg, chat_id=1)
...
async def test_watch_add_list_remove_flow(watchlist_engine) -> None:
    # add
    msg1 = FakeMessage()
    update1 = FakeUpdate(message=msg1, chat_id=1)
    context1 = FakeContext(args=["add", "2330"])
    await handlers.watch_command(update1, context1)
    assert msg1.reply_text_calls[-1].text == "✅ 已加入：2330"

    # add duplicate
    msg2 = FakeMessage()
    update2 = FakeUpdate(message=msg2, chat_id=1)
    ...
    # list contains
    msg3 = FakeMessage()
    update3 = FakeUpdate(message=msg3, chat_id=1)
    ...
    # remove
    msg4 = FakeMessage()
    update4 = FakeUpdate(message=msg4, chat_id=1)
    ...
    # list empty
    msg5 = FakeMessage()
    update5 = FakeUpdate(message=msg5, chat_id=1)
...
async def test_watch_is_per_chat(watchlist_engine) -> None:
    # add to chat 1
    msg1 = FakeMessage()
    await handlers.watch_command(FakeUpdate(message=msg1, chat_id=1), FakeContext(args=["add", "TSLA"]))
    assert msg1.reply_text_calls[-1].text == "✅ 已加入：TSLA"

    # list in chat 2 is empty
    msg2 = FakeMessage()
    await handlers.watch_command(FakeUpdate(message=msg2, chat_id=2), FakeContext(args=["list"]))
