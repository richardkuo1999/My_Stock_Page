...
class StockSelector:
    ...
    async def fetch_institutional_top50(self, session=None) -> list[str]:
        ...
        try:
            tasks = [
                self.fetch_investor_stocks(session, i_type)
                for i_type in INVESTOR_TYPES.values()
            ]
    ...
    async def get_invest_anchors(self, session=None) -> list[str]:
        ...
        with Session(engine) as db_session:
            config = db_session.exec(select(SystemConfig).where(SystemConfig.key == "investanchors")).first()
            return self._parse_list(config.value) if config else []

    async def get_user_choice(self, session=None) -> list[str]:
        with Session(engine) as db_session:
            config = db_session.exec(select(SystemConfig).where(SystemConfig.key == "user_choice")).first()
            return self._parse_list(config.value) if config else []
            
    async def get_target_etfs(self, session=None) -> list[str]:
        """Fetch target ETFs from SystemConfig."""
        with Session(engine) as db_session:
            config = db_session.exec(select(SystemConfig).where(SystemConfig.key == "target_etfs")).first()
            return self._parse_list(config.value) if config else []
