...
**通知：** 定時同步偵測到變更時，推播給所有 `/sub_wlist` 訂閱者，訊息包含新增/更新/移除摘要及完整自選股清單（與 `/wlist` 同格式）。

**Database:** `GSheetSubscription`（註冊資訊）, `WatchlistEntry`（同步後的持股）

**Services:** `gsheet_monitor.gsheet_sync_job()`（定時任務）, `gsheet_monitor.gsheet_sync_for_user()`（個人同步）, `gsheet_monitor.gsheet_sync_for_chat()`（聊天室同步）

**權限：**
- `add` / `del` — 各使用者管理自己的試算表
- `list` / `sync` — 群組內任何人都可查看和同步所有已註冊的試算表
