...
class TestStockAnalyzer:
    ...
    async def test_analyze_stock_with_valid_ticker(
        self, analyzer, mock_stock_data, mock_stock_info
    ):
        with (
            patch.object(analyzer.fetcher, "fetch_yahoo_data") as mock_fetch,
            patch.object(analyzer.finmind, "get_per_pbr") as mock_per_pbr,
            patch.object(analyzer.finmind, "get_stock_info") as mock_info,
            patch.object(analyzer.anue, "fetch_estimated_data") as mock_anue,
        ):
            mock_fetch.return_value = {"history": mock_stock_data, "info": mock_stock_info}
    ...
    async def test_analyze_stock_with_empty_history(self, analyzer, mock_stock_info):
        with patch.object(analyzer.fetcher, "fetch_yahoo_data") as mock_fetch:
            ...
            mock_fetch.return_value = {"history": empty_df, "info": mock_stock_info}
