...
class TestStockAPI:
    def test_analyze_stock_with_valid_ticker(self, client, auth_headers):
        with patch(
            "analysis_bot.services.stock_service.StockService.get_or_analyze_stock"
        ) as mock_analyze:
...
class TestSettingsAPI:
    def test_toggle_tag(self, client, auth_headers):
        with patch(
            ...
            response = client.post(
                "/settings/tags/toggle",
                json={"tag": "favorite", "enable": True},
    ...
    def test_update_list(self, client, auth_headers):
        with patch(
            "analysis_bot.services.stock_service.StockService.set_system_config"
        ) as mock_update:
            response = client.post(
                "/settings/lists/update",
                json={"key": "user_choice", "value": "2330 2317"},
                headers=auth_headers,
            )
